#!/usr/bin/env python3
"""
Pull EDHREC JSON for a commander or batch of commanders.

Writes:
- data/raw_edhrec/<slug>.json     (raw EDHREC JSON for caching)
- data/datasets/edhrec_commander_cards.csv
  (rows: commander,card,percent,synergy,section)

Usage:
  python tools/pull_edhrec.py --commander "Kaalia of the Vast"
  python tools/pull_edhrec.py --batch commanders.txt --sleep 0.6
"""
...
UA = {"User-Agent": "mtg-commander-builder/0.1 (personal use)"}
...
def parse_cards(data: dict):
    """Extract rows (name, percent, synergy, section) from EDHREC JSON."""
    rows = []

    def add(label, card):
        name = card.get("name") or card.get("card", {}).get("name")
        if not name: return
        try: pct = float(card.get("p", card.get("percent", 0)))
        except: pct = 0.0
        try: sy = float(card.get("synergy", card.get("synergyScore", 0)))
        except: sy = 0.0
